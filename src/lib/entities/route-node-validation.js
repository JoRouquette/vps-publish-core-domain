...
exports.validateRouteTree = validateRouteTree;
exports.getNodeConflicts = getNodeConflicts;
exports.hasNodeConflicts = hasNodeConflicts;
/**
 * Validates a route tree for conflicts and duplicates
 */
function validateRouteTree(routeTree) {
    const conflicts = [];
    const pathRegistry = new Map(); // Track all full paths
    const processedDuplicates = new Set(); // Track already reported duplicate segments
    ...
    const rootRoutesWithEmptySegment = routeTree.roots.filter((root) => !root.segment || root.segment === '' || root.segment === '/');
    ...
    function validateNode(node, parentPath, siblings) {
        ...
        // 2. Build full path
        const currentPath = parentPath === '' ? `/${normalizedSegment}` : `${parentPath}/${normalizedSegment}`;
        ...
        const duplicatesAtLevel = normalizedSiblings.filter((sibling) => sibling.id !== node.id && sibling.normalizedSegment === normalizedSegment);
        if (duplicatesAtLevel.length > 0) {
            ...
            if (!processedDuplicates.has(duplicateGroupKey)) {
                ...
                const allDuplicates = normalizedSiblings.filter((s) => s.normalizedSegment === normalizedSegment);
                conflicts.push({
 ...
 */
function getNodeConflicts(routeTree, nodeId) {
    const result = validateRouteTree(routeTree);
    return result.conflicts.filter((conflict) => conflict.conflictingNodes.some((n) => n.id === nodeId));
}
/**
 * Check if a specific node has conflicts
 */
function hasNodeConflicts(routeTree, nodeId) {
